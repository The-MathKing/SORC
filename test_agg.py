...
# Use smallest (which gave 0.94 in SVD!)
# Wait, SVD uses smallest eigenvectors of L?
# SVD of adjacency matrix uses LARGEST eigenvectors of A.
# L = D - A. Largest eigs of A are SMALLEST eigs of L (assuming regular graph).
...
def score_max(u, v):
    ...
    return 1.0 - np.max([wasserstein_distance(F_norm[su, i], F_norm[sv, i], wu, wv) for i in range(F_norm.shape[1])])
...
def score_l2(u, v):
    su, wu = node_supports[u]
    sv, wv = node_supports[v]
    return 1.0 - np.sqrt(np.sum([wasserstein_distance(F_norm[su, i], F_norm[sv, i], wu, wv)**2 for i in range(F_norm.shape[1])]))

def score_svd_like(u, v):
    su, wu = node_supports[u]
    sv, wv = node_supports[v]
    # just dot product of the means
    mu = [np.sum(F_norm[su, i] * wu) for i in range(F_norm.shape[1])]
    mv = [np.sum(F_norm[sv, i] * wv) for i in range(F_norm.shape[1])]
    return np.dot(mu, mv)

for name, func in [("Max", score_max), ("Mean", score_mean), ("L2", score_l2), ("Dot", score_svd_like)]:
